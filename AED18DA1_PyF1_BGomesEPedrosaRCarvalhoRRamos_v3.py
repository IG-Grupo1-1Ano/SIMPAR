...
def menu():
    ...
    print("\n")
    print("""       2º Semestre - Informatica De Gestão

        Selecione os parametros da simulação: 

    [1] Número maximo de passageiros
    [2] Número máximo de bagagens permitido por passageiro
    [3] Número de balcões abertos para atendimento e despacho de bagagem
    [4] Ciclos de tempo em que a simulação decorre
    [5] Percentagem de passageiros criados por ciclo
    
    [6] Consultar parametros escolhidos
...
def simpar_simula(num_pass, num_bag, num_balcoes, ciclos, p_enche):
    ...
    :return: None
    """
    print(str(num_pass) + ' - ' + str(num_bag) + ' - ' + str(num_balcoes) + ' - ' + str(ciclos) + ' - ' + str(p_enche))
    ...
    for ciclo in range(0, ciclos):
        ...
        if num_pass > 0:
            ...
            for n_balcao in range(1,
                                  num_balcoes + 1):  # aqui precorremos todos os bacções para colocar pessoas na fila
                ...
                if probabilidade >= temp:
                    ...
                    balcoes_filas_curtas = [balcao for balcao in balcoes if
                                            balcao.obtem_fila().size() == fila_mais_curta]
...
# **************************** Funções de Introdução de Valores ***************************** #

def parpass():  # simpar_simula(num_pass, num_bag, num_balcoes, ciclos, p_enche)
    limpa()
    limpa()
    ...
    while True:
        ...
        try:
            passag = 0
            a = passag
            aux = int(input("O valor default é " + str(
                a) + ", indique o novo valor: "))  # display do valor antigo
            print("O valor passou de: " + str(a))
            print("Para: " + str(aux))
            if aux != a:  # se diferente substiui
                passag = aux
            input()
...
def choose():
    ...
    while True:
        ...
        if escolha == 1:
            ...
            print(str(num_pass) + ' - ' + str(num_bag) + ' - ' + str(num_balcoes) + ' - ' + str(ciclos) + ' - ' + str(
                p_enche))
        elif escolha == 7:
            go()
...
# **************************** Função de arranque da simulação ***************************** #

def go():
    if __name__ == "__main__":
        simpar_simula(passag, bagg, balc, cic, pench)
